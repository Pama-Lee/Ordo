...
onMounted(async () => {
  ...
  const series = Object.entries(DATA.engines).map(([name, { qps, color }]) => ({
    ...
    markPoint: name === 'Ordo (Rust)' ? {
      data: [{
        ...
        label: { formatter: '~60K\n饱和点', fontSize: 10, fontWeight: 'bold' }
      }]
    } : undefined,
  }))
  ...
  const qpsOption = {
    ...
    title: {
      text: '规则引擎 QPS 随并发数变化',
      subtext: '4 分支决策规则 • Apple M1 Pro • 每级 10 秒',
      left: 'center',
        ...
        if (ordoVal) {
          ...
          for (const p of params) {
            if (!p.seriesName.includes('Ordo')) {
              ...
              html += `Ordo 比 ${p.seriesName.split(' ')[0]} 快 <b>${ratio}x</b><br/>`
            }
          }
        }
        return html
...
所有测试使用 [`hey`](https://github.com/rakyll/hey)，每个并发级别测试 10 秒，环境为 Apple M1 Pro (10 核, 16GB RAM)。

| 引擎              | C=1    | C=5    | C=10   | C=25   | C=50   | C=100  | C=150  | C=200  | C=300  | C=500  |
| ----------------- | ------ | ------ | ------ | ------ | ------ | ------ | ------ | ------ | ------ | ------ |
...
完整基准测试数据请查看[性能基准测试](./benchmarks.md)。
